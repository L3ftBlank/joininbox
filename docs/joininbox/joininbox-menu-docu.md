...
┌──────────────Quickstart options─────────────────┐
│ ┌─────────────────────────────────────────────┐ │
│ │<a href="#mm-011">GEN</a>     Generate a new wallet                │ │
...
| Option | Explanation |
|--------|---------------------------|
| GEN | **Generate Wallet** - Creates a new JoinMarket wallet from scratch. Unlike standard Bitcoin wallets, a JoinMarket wallet is pre-divided into 5 "mixdepths" (0-4). These act as separate compartments to isolate funds at different privacy levels, which is essential for effective coinjoins. Choose a strong password and store the 12 word recovery seed safely offline. |
...
<details>
<summary>Generate a new wallet</summary>

Creates a new JoinMarket wallet from scratch. Unlike standard Bitcoin wallets, a JoinMarket wallet is pre-divided into 5 "mixdepths" (0-4). These act as separate compartments to isolate funds at different privacy levels, which is essential for effective coinjoins. Choose a strong password and store the 12 word recovery seed safely offline. 
</details>

![Terminal dialog: Generate new wallet](/media/GEN-dialog2.png)
> *JoininBox Main Menu – Navigate JoinMarket features with a simple graphical interface*

---

### GEN Dialog Walkthrough

#### 1. User Data Location
```
This is the standard path, where the wallets are saved: /home/joinmarket/.joinmarket/
```
Default directory for all JoinMarket data. Contains wallet files, configuration, and logs. **Contains private keys – secure this directory.**

---

#### 2. Two-Factor Mnemonic
```
Would you like to use a two-factor mnemonic recovery phrase? (y/n)
```
| Option | When to Use |
|--------|-------------|
| `n` | Standard 12-word seed (recommended for most users) |
| `y` | Adds a passphrase to your seed for extra security |

**Warning:** If you choose `y` and forget the passphrase, your wallet cannot be recovered.
...
```
Password required to unlock your wallet file. **If forgotten, your funds are permanently inaccessible.**

**Recommendation:** Use a strong, unique password stored in a password manager.
...
```
Each file is a separate wallet. Use meaningful names:
- `wallet.jmdat` – Default
- `trading.jmdat` – Active trading
- `savings.jmdat` – Long-term storage

---

#### 5. Fidelity Bonds
```
Would you like this wallet to support fidelity bonds? (y/n)
```
| Option | When to Use |
|--------|-------------|
| `n` | You only send payments (Taker) |
| `y` | You plan to run a Yield Generator (Maker) |

Fidelity bonds lock Bitcoin for a period to prove legitimacy and earn higher fees.

---

#### 6. Recovery Mnemonic
```
useful benefit road kiss twin spring play poet stem benefit spike budget
```
Your 12-word recovery seed. **Write this down on paper immediately.**

| Do | Don't |
|----|-------|
| Write on paper | Store in cloud/email |
| Store in safe | Take a photo |
| Verify spelling | Share with anyone |
| Test recovery | Type in password manager |
...
Your wallet is created. Use **M0** or **DISPLAY** to see your deposit addresses.
...
│                                                                │
├────────────────────────────────────────────────────────────────┤
│                     <a href="#top">< Yes ></a>         <a href="#top">< No ></a>                     │
└────────────────────────────────────────────────────────────────┘
</div>
